...
class DDQN(OffPolicyAlgorithm):
    ...
    def __init__(
        self,
        policy: Union[str, Type[DDQNPolicy]],
    ...
    def train(self, gradient_steps: int, batch_size: int = 100) -> None:
        ...
        for _ in range(gradient_steps):
            ...
            with th.no_grad():
                ...
                next_q_values, next_actions = next_q_values.max(dim=1)

                next_q_values_from_current = self.q_net(replay_data.next_observations)
                best_next_q_values_from_current = th.gather(next_q_values_from_current, dim=1, index=next_actions.long().unsqueeze(1))

                #calculate q function variance over time
                q_diffs_for_batch = th.abs(best_next_q_values_from_current - next_q_values.unsqueeze(1))
        ...
        print(np.mean(q_diffs_for_all_batches))
