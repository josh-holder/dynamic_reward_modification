...
class DRM(OffPolicyAlgorithm):
    ...
    def collect_rollouts(
        ...
        while should_collect_more_steps(train_freq, num_collected_steps, num_collected_episodes):
            ...
            # qs_of_rollout = th.cat(self.critic(self._last_obs, actions), dim=1)
            # print(self._last_obs)
            # print(type(self._last_obs))
            # print(type(actions))
            # print(type(th.from_numpy(self._last_obs)),self._last_obs.dtype)
            # print(type(th.from_numpy(actions)),actions.dtype)
            actions_for_q_function = th.from_numpy(actions).float()
            qs_of_rollout = self.critic(th.from_numpy(self._last_obs), actions_for_q_function)

            avg_q_rollout = th.cat(qs_of_rollout, dim=1).mean()

            if i % 25 == 0:
                self.logger.record("rollout/q_val", avg_q_rollout.item())
